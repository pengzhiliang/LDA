...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __init__(self, mode, cfg, root='./datasets/imbalance_cifar10', imb_type='exp',
        ...
        if self.train:
            ...
            img_num_list = self.get_img_num_per_cls(self.cls_num, imb_type, cfg.DATASET.IMBALANCECIFAR.RATIO)
            self.transform = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
            ])
        ...
        class_cate_index = np.zeros(self.cls_num).astype(np.int)  # few
        class_cate_index[np.array(img_num_list) >= 20] = 1  # medium
        class_cate_index[np.array(img_num_list) > 100] = 2  # many
        self.class_cate_index = class_cate_index

        if self.cfg.CLASSIFIER.TYPE == 'LDA' and self.train:
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.CLASSIFIER.TYPE == 'LDA' and self.train:
            meta['class_weight'] = self.class_weight_for_lda[target]
        meta['shot_cate'] = self.class_cate_index[target]
