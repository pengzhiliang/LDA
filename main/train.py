...
if __name__ == "__main__":
    ...
    if hasattr(valid_set, 'class_cate_index'):
        valid_set.class_cate_index = train_set.class_cate_index
    ...
    num_class_list = train_set.get_cls_num_list()
    logger.info("The number of each class:{}".format(np.sort(np.array(num_class_list))))
    ...
    combiner = Combiner(cfg, device, num_classes=num_classes, num_class_list=num_class_list)
    if cfg.CLASSIFIER.TYPE == 'LDA':
        extra_classifier_shape = model.module.classifier[0].weight.shape
        extra_num_list = torch.tensor(num_class_list).float().to(device)
        optimizer = get_optimizer(cfg, model, combiner, num_class_list=extra_num_list,
                              classifier_shape=extra_classifier_shape)
    else:
        optimizer = get_optimizer(cfg, model, combiner)
    ...
    logger.info("=> output model will be saved in {}".format(model_dir))
    ...
    if len(all_models) <= 1 or auto_resume == False:
        ...
        resume_model_path = os.path.join(model_dir, 'latest_model_checkpoint.pth')
    ...
    logger.info("-------------------Train start :{} -------------------".format(cfg.BACKBONE.TYPE))
    ...
    for epoch in range(start_epoch, epoch_number + 1):
        ...
        model_save_path = os.path.join(model_dir, 'latest_model_checkpoint.pth')
